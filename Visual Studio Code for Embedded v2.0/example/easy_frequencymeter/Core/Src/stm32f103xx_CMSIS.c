...
void CMSIS_TIM3_init(void) {
    ...
    SET_BIT(TIM3->CR1, TIM_CR1_OPM);                                  // One pulse mode off(Счетчик не останавливается при обновлении)
    SET_BIT(TIM3->CR1, TIM_CR1_DIR);                                  // Считаем вниз
    ...
    MODIFY_REG(TIM3->CR2, TIM_CR2_MMS_Msk, 0b010 << TIM_CR2_MMS_Pos);  // Сделать режим мастера. По обновлению таймера будет запускаться триггер на запуск других таймеров
...
void CMSIS_TIM4_init(void) {
    ...
    CLEAR_BIT(TIM4->CCMR1, TIM_CCMR1_CC1S);
    CLEAR_BIT(TIM4->CCER, TIM_CCER_CC1P);
    MODIFY_REG(TIM4->SMCR, TIM_SMCR_TS_Msk, 0b010 << TIM_SMCR_TS_Pos);    // 010 Запуск по таймеру 3
    MODIFY_REG(TIM4->SMCR, TIM_SMCR_SMS_Msk, 0b101 << TIM_SMCR_SMS_Pos);  // 101 Gated_Mode

    MODIFY_REG(TIM4->SMCR, TIM_SMCR_ETF_Msk, 0b1111 << TIM_SMCR_ETF_Pos);  // 1111: fSAMPLING=fDTS/32, N=8
    MODIFY_REG(TIM4->SMCR, TIM_SMCR_ETPS_Msk, 0b00 << TIM_SMCR_ETPS_Pos);  // 00: Prescaler OFF
    CLEAR_BIT(TIM4->SMCR, TIM_SMCR_ECE);                                   // 0: External clock mode 2 disabled
    CLEAR_BIT(TIM4->SMCR, TIM_SMCR_ETP);                                   // 0: ETR is non-inverted, active at high level or rising edge
    NVIC_EnableIRQ(TIM4_IRQn);                                             // Разрешить прерывания по таймеру 4
    SET_BIT(TIM4->CR1, TIM_CR1_CEN);                                       // Запуск таймера
}

void CMSIS_TIM3_init_Gated_Mode(void) {
    /*Включим тактирование таймера (страница 48)*/
    SET_BIT(RCC->APB1ENR, RCC_APB1ENR_TIM3EN);  // Запуск тактирования таймера 3
    SET_BIT(RCC->APB2ENR, RCC_APB2ENR_AFIOEN);  // Запуск тактирования альтернативных функций
    ...
    MODIFY_REG(TIM3->CR1, TIM_CR1_CMS_Msk, 0b00 << TIM_CR1_CMS_Pos);  // Выравнивание по краю
    CLEAR_BIT(TIM3->CR1, TIM_CR1_ARPE);                               // Auto-reload preload enable
    MODIFY_REG(TIM3->CR1, TIM_CR1_CKD_Msk, 0b00 << TIM_CR1_CKD_Pos);  // Предделение выключено

    TIM3->CR2 = 0;

    // Пример режима Slave Gated mod
    MODIFY_REG(TIM3->SMCR, TIM_SMCR_SMS_Msk, 0b101 << TIM_SMCR_SMS_Pos);  // Gated mode
    MODIFY_REG(TIM3->SMCR, TIM_SMCR_TS_Msk, 0b101 << TIM_SMCR_TS_Pos);    // 101: Filtered Timer Input 1 (TI1FP1) (Таймер 3, канал 1)
    SET_BIT(TIM3->SMCR, TIM_SMCR_MSM);
    MODIFY_REG(TIM3->SMCR, TIM_SMCR_ETF_Msk, 0b1111 << TIM_SMCR_ETF_Pos);  // 1111: fSAMPLING=fDTS/32, N=8
    MODIFY_REG(TIM3->SMCR, TIM_SMCR_ETPS_Msk, 0b00 << TIM_SMCR_ETPS_Pos);  // 00: Prescaler OFF
    CLEAR_BIT(TIM3->SMCR, TIM_SMCR_ECE);                                   // 0: External clock mode 2 disabled
    CLEAR_BIT(TIM3->SMCR, TIM_SMCR_ETP);                                   // 0: ETR is non-inverted, active at high level or rising edge

    // Настройка PA6 на вход гейта
    ...
    TIM3->PSC = 9 - 1;                                                 // 8 МГц
    TIM3->ARR = 0xFFFF;                                                // 16 бит
    MODIFY_REG(TIM3->CR2, TIM_CR2_MMS_Msk, 0b001 << TIM_CR2_MMS_Pos);  // Генерировать TRGO по событию обновления (Update)
