...
//#define FreeRTOS_USE //Использовать FreeRTOS
	...
	void CMSIS_TIM1_init(void); //Инициализация таймера 3. Включение прерывания по переполнению
	void CMSIS_TIM1_PWM_CHANNEL1_init(void); //Запуск шим канала 1 (PA8 - Pin)
	void CMSIS_TIM1_PWM_CHANNEL2_init(void); //Запуск шим канала 2 (PA9 - Pin)
	void CMSIS_TIM3_init(void); //Инициализация таймера 3. Включение прерывания по переполнению
    void CMSIS_TIM3_init_Gated_Mode(void); //Инициализация таймера 3. Пример режима Gated mode.
