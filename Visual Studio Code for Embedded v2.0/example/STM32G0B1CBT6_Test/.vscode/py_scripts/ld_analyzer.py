#   ld analyzer v 1.2 (Скрипт еще сырой. Пытаюсь сделать универсальным для ARM и RISC-V. Большую часть писал Deep Seek)
#   Автор: Волков Олег
#   Дата создания скрипта: 13.11.2025
...
def analyze_linker_script(file_path, output_format="text"):
    ...
    common_sections = [
        ...
        (r'\.data\s*:\s*\{[^}]+\}\s*>RAM_D1\s+AT>\s*FLASH', '.data', 'RAM_D1', 'FLASH'),
        (r'\.bss\s*:\s*\{[^}]+\}\s*>RAM_D1', '.bss', 'RAM_D1', 'RAM_D1'),
        (r'\._user_heap_stack\s*:\s*\{[^}]+\}\s*>RAM_D1', '._user_heap_stack', 'RAM_D1', 'RAM_D1'),
        (r'\.sram1\s*\(?\s*NOLOAD\s*\)?\s*:\s*\{[^}]+\}[^}]*>\s*RAM_D1', '.sram1', 'RAM_D1', 'RAM_D1'),
        (r'\.sram2\s*\(?\s*NOLOAD\s*\)?\s*:\s*\{[^}]+\}[^}]*>\s*RAM_D2', '.sram2', 'RAM_D2', 'RAM_D2'),
        (r'\.sram3\s*\(?\s*NOLOAD\s*\)?\s*:\s*\{[^}]+\}[^}]*>\s*RAM_D3', '.sram3', 'RAM_D3', 'RAM_D3'),
        (r'\.itcmram\s*\(?\s*NOLOAD\s*\)?\s*:\s*\{[^}]+\}[^}]*>\s*ITCMRAM', '.itcmram', 'ITCMRAM', 'ITCMRAM'),
        (r'\.dtcmram\s*\(?\s*NOLOAD\s*\)?\s*:\s*\{[^}]+\}[^}]*>\s*DTCMRAM', '.dtcmram', 'DTCMRAM', 'DTCMRAM'),
        (r'\.sdram\s*\(?\s*NOLOAD\s*\)?\s*:\s*\{[^}]+\}[^}]*>\s*SDRAM', '.sdram', 'SDRAM', 'SDRAM'),
        (r'\.sram1\s*\(NOLOAD\)\s*:\s*\{[^}]+\}\s*>RAM_D1', '.sram1', 'RAM_D1', 'RAM_D1'),
        (r'\.sram2\s*\(NOLOAD\)\s*:\s*\{[^}]+\}\s*>RAM_D2', '.sram2', 'RAM_D2', 'RAM_D2'),
        (r'\.sram3\s*\(NOLOAD\)\s*:\s*\{[^}]+\}\s*>RAM_D3', '.sram3', 'RAM_D3', 'RAM_D3'),
        (r'\.itcmram\s*\(NOLOAD\)\s*:\s*\{[^}]+\}\s*>ITCMRAM', '.itcmram', 'ITCMRAM', 'ITCMRAM'),
        (r'\.dtcmram\s*\(NOLOAD\)\s*:\s*\{[^}]+\}\s*>DTCMRAM', '.dtcmram', 'DTCMRAM', 'DTCMRAM'),
        (r'\.sdram\s*\(NOLOAD\)\s*:\s*\{[^}]+\}[^}]*>\s*SDRAM', '.sdram', 'SDRAM', 'SDRAM'),
        # RISC-V секции
