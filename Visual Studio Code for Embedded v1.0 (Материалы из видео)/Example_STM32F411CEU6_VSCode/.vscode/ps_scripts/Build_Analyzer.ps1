...
$ram_size_ld_file = "128"

# Размер памяти FLASH (KB) Смотри ld файл
$flash_size_ld_file = "512"
...
$ARMLine = $sizeOutput | Select-String -Pattern '\.ARM'
$preinit_arrayLine = $sizeOutput | Select-String -Pattern '\.preinit_array'
...
if ($isr_vectorLine -and $textLine -and $dataLine -and $rodataLine -and $init_arrayLine -and $fini_arrayLine -and $dataLine -and $bssLine -and $_user_heap_stackLine) {
    ...
    $ARM = ($ARMLine -split '\s+')[1]
    $preinit_array = ($preinit_arrayLine -split '\s+')[1]
    ...
    [int]$flashUsed = $isr_vector + $rodata + $init_array + $fini_array+ $text + $data + $ARM + $preinit_array
